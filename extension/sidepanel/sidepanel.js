...
function addLogEntry(type, status, result) {
  const entry = {
  ...
  return `<div class="workflow-card" data-wf-id="${wfId}">
    <div class="wf-header">
      ...
      <div class="wf-actions">
        <button class="btn btn-exec" data-action="run-workflow" data-wf-id="${wfId}">全部执行</button>
...
async function runWorkflow(wfId) {
  ...
  const runner = new WorkflowRunner(items, {
    onStep: (stepIndex, status, result) => {
      ...
      addLogEntry(type, status, result || "");
    },
  });
