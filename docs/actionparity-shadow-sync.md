...
| 方案 | 10 分钟总流量 | 相对影核 SSE | 数据来源 |
| --- | --- | --- | --- |
| **影核动作同步 · SSE** | **20.7 KB** | 1× | 实测 |
| 影核动作同步 · 轮询 3s | 153.0 KB | 7.4× | 实测 |
| 屏幕流 · 静止画面 0.3 Mbps | 21.46 MB | 1061× | 按码率估算 |
| 屏幕流 · 轻度操作 1.5 Mbps | 107.29 MB | 5306× | 按码率估算 |
| 屏幕流 · 持续操作 4 Mbps | 286.10 MB | 14150× | 按码率估算 |
...
有意思的是轮询那一行里的 45%：影核的流量已经小到**HTTP 头比内容还大**。所以真正
的优化不是再压缩信封，而是别再每三秒问一次。

## 同一条流，换一种送法

```text
GET /v3/sync/tasks/:taskId/stream
GET /v3/sync/tasks/:taskId/stream?after=<opaque-cursor>
```

SSE 长连接，语义和轮询接口**完全一致**：同样的 `action-parity/sync@0.1` 信封、
同样的不透明游标、同样的至少一次投递。区别只是 relay 有变化才发。

- 连上先把游标之后欠的补齐，再进入等待 —— 否则建连瞬间的事件会漏掉；
- 每 25 秒一个 SSE 注释保活，否则中间设备会静默掐断空闲连接；
- 客户端断开就退订，不留死连接；
- 手机端断流后**自动退回轮询**，游标不变，所以两条路径随时可以互换，
  不需要第二套恢复规则。

代价也说清楚：SSE 只在 App 处于前台（连接活着）时有效。App 被系统杀掉之后的
唤醒仍然要 FCM/APNs，那需要 Apple/Google 的凭据，是另一半问题。
